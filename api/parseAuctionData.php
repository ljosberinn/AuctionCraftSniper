...
if (!isset($decodedPOST['step']) && empty($decodedPOST['recipeIDs'])) {
    ...
    $expansionLevel = $AuctionCraftSniper->isValidExpansionLevel ((int) $decodedPOST['expansionLevel']);

    if ($expansionLevel && $expansionLevel === 0) {
        $expansionLevel = 8;
    }
...
if (file_exists ($fileName) && $stream = fopen ($fileName, 'r')) {
    ...
    if ((int) $response['step'] === (int) $response['reqSteps']) {
        ...
        if ($AuctionCraftSniper->isValidExpansionLevel ($expansionLevel) && $AuctionCraftSniper->isValidHouse ($decodedPOST['house'])) {
