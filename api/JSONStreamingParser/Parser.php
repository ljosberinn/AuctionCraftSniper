...
class Parser
{
    ...
    public function parse($itemArr)
    {
        ...
        while (!feof($this->stream) && !$eof) {
            ...
            for ($i = 0; $i < $byteLen; $i++) {
                ...
                $this->consumeChar($line[$i], $itemArr);
                $this->charNumber++;
     ...
     * @throws ParsingError
     */
    private function consumeChar($c, $itemArr)
    {
    ...
    private function checkAndSkipUtfBom($c) {
        ...
        if ($this->utfBom == self::UTF16_BOM && $this->charNumber == 2 && $c == chr(254)) {
            $this->utfBom = self::UTF32_BOM;
        }
